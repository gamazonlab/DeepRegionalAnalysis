...
#clumping
# cmd<-paste0('plink --bfile /data/c***/z***/data/gtex/geno/v8/liftover/pruning/gtex_rsid_prune0.9 --extract /data/c***/z***/data/gtex/geno/v8/liftover/pruning/gtex_rsid_prune0.9_non_palindromic.bim --clump /data/g***/z***/covid19_neanderthals/covid19_gwas/raw/covid19hg/COVID19_HGI_A2_ALL_20200930.b37.txt.gz --clump-field all_inv_var_meta_p --clump-snp-field rsid --clump-p1 1 --clump-r2 ',r2_cutoff,' --out /data/g***/z***/covid19_neanderthals/mrjti/clumping/',pheno,'_',r2_cutoff)
# system(cmd,wait = T)
# 
clumped<-read.table(paste0('/data/g***/z***/covid19_neanderthals/mrjti/clumping/',pheno,'_',r2_cutoff,'.clumped'),header = T,stringsAsFactors = F)

#load gwas result
# gwas<-read.table('/data/g***/z***/covid19_neanderthals/covid19_gwas/raw/covid19hg/COVID19_HGI_A2_ALL_20200930.b37.txt.gz',header = T,stringsAsFactors = F,comment.char = '&')
# gwas<-gwas[,c('rsid','ALT','REF','all_inv_var_meta_beta','all_inv_var_meta_sebeta','all_inv_var_meta_p')]
# saveRDS(gwas,'/data/g***/z***/covid19_neanderthals/covid19_gwas/raw/covid19hg/COVID19_HGI_A2_ALL_20200930.b37.rds')
gwas<-readRDS('/data/g***/z***/covid19_neanderthals/covid19_gwas/raw/covid19hg/COVID19_HGI_A2_ALL_20200930.b37.rds')
...
twas<-read.csv(paste0('/data/g***/z***/covid19_neanderthals/asso/covid19hg_112020/xt_covid19hg_112020.csv'),header = T,stringsAsFactors = F)
...
if (i_start<length(gene_list)){  #68
  for (i in i_start:i_end){
    ...
    output[i,'n_eqtl']<-length(sapply(df_mr$rsid, function(x) which(colnames(d)==x)))
    if(output[i,'n_eqtl']==1){next}
    ...
    set.seed(as.numeric(sub('ENSG','',gene))+100) 
    fit<-try(TRB_LASSO(x=x,y=y,intercept = T,standardize = T,nfolds = nf,alpha=0.05/length(gene_list),weights=rep(1,nrow(df)),penalty.factor=penalty.factor,B=500)) ######
  ...
  #output
  write.csv(output,paste0('/data/g***/z***/covid19_neanderthals/mr/result/covid19hg_112020/r2_',r2_cutoff,'_',args,'.csv'),quote = F,row.names = F)
